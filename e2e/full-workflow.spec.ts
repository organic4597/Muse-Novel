...
test.describe.serial('소설 작성 전체 워크플로우', () => {
  ...
  test('캐릭터 추가', async () => {
    ...
    await expect(page.getByText(TEST_CHARACTER.name)).toBeVisible();
    await expect(page.getByText(TEST_CHARACTER.role)).toBeVisible();
  });
  ...
  test('챕터 생성 및 에디터 작성', async () => {
    ...
    // Verify the "챕터를 선택하세요" placeholder is visible
    await expect(page.getByText('챕터를 선택하세요')).toBeVisible();
  ...
  test('정리 — 프로젝트 삭제', async () => {
    ...
    await expect(page.getByText(TEST_PROJECT.title)).toBeHidden();
  });
});
