...
const STORY_PLANNING_PHASE_CONFIG: Record<StoryPlanningPhase, StoryPlanningPhaseConfig> = {
  ...
  plot: {
    summary: (draft) => {
      const parts = [
        ...
        quoteValue(draft.endingDirection),
        quoteValue(draft.firstChapterOutline),
      ].filter(Boolean);
      return parts.length > 0 ? `플롯 방향은 ${parts.join(', ')}로 정리됐어요.` : '플롯과 첫 장면 방향을 정리하고 있어요.';
    },
    question: '주요 사건의 인과와 결말 방향을 정하고, 첫 챕터를 목표·갈등·전환·결과의 장면 비트로 나눠볼까요?',
    followUp: '주인공이 가장 크게 흔들리는 전환점과 그 선택의 대가를 먼저 잡으면 플롯이 덜 반복돼요. 어느 쪽부터 선명하게 할까요?',
    options: ['3막 구조로 정리하기', '중반 반전 넣기', '결말 방향 정하기', '첫 챕터 장면 비트 만들기'],
    buildContextOptions: (draft) => {
      const contextual: string[] = [];
      ...
      if (draft.endingDirection) {
        contextual.push(`${draft.endingDirection} 결말로 인과 정리하기`);
      }
  ...
  writing_setup: {
    summary: (draft) => {
      const parts = [
        quoteValue(draft.targetAudience),
        quoteValue(draft.pointOfView),
        quoteValue(draft.narrativeTense),
        quoteValue(draft.writingStyle),
      ].filter(Boolean);
      return parts.length > 0
        ? `집필 기준은 ${parts.join(', ')}로 정리됐어요.`
        : '집필 전에 독자층, 시점, 시제와 문체를 고정하고 있어요.';
    },
    question: '집필할 때 계속 유지할 독자층, 서술 시점·시제와 문체 규칙을 정해볼까요?',
    followUp: '이 기준은 기획을 작품으로 만들 때 집필 설정으로 저장됩니다. 피하고 싶은 표현이나 이번 작품에서 반드시 지킬 원칙도 있나요?',
    options: [
      '성인 독자 · 3인칭 제한 · 과거형',
      '청소년 독자 · 1인칭 · 현재형',
      '문체 규칙 구체화하기',
    ...
    buildContextOptions: (draft) => {
      const contextual: string[] = [];
      if (draft.tone) contextual.push(`${draft.tone} 분위기를 문체 규칙으로 변환하기`);
      if (draft.targetAudience) contextual.push(`${draft.targetAudience} 기준으로 수위 조정하기`);
      return contextual;
    },
  },
