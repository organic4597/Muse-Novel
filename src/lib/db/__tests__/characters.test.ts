...
describe('Character Queries', () => {
  ...
  describe('deleteCharacter', () => {
    ...
    it('should delete dependent relationships and gallery images', async () => {
      const character = await createCharacter(db, {
        projectId,
        name: '삭제 대상',
      });
      const other = await createCharacter(db, {
        projectId,
        name: '상대 인물',
      });
      db.insert(schema.characterRelationships)
        .values({
          characterAId: character.id,
          characterBId: other.id,
          relationshipType: '동료',
        })
        .run();
      ...
      expect(
        db
          ...
          .where(eq(schema.characterRelationships.characterAId, character.id))
          .all()
      ).toHaveLength(0);
      expect(
        db
          ...
          .where(eq(schema.characterImages.characterId, character.id))
          .all()
      ).toHaveLength(0);
    });
  });
