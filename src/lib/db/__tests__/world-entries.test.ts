...
describe('World Entry Queries', () => {
  ...
  describe('listWorldEntries', () => {
    ...
    it('loads all entry tags without one request per entry', async () => {
      const first = await createWorldEntry(db, {
        projectId,
        category: '종파',
        title: '소림',
      ...
      db.insert(schema.worldEntryTags).values([
        { entryId: first.id, tag: '불가' },
        { entryId: first.id, tag: '권법' },
        { entryId: second.id, tag: '도가' },
      ]).run();

      const result = await listWorldEntriesWithTags(db, projectId);
      const byTitle = new Map(result.map((entry) => [entry.title, entry.tags]));

      expect(byTitle.get('소림')?.map((tag) => tag.tag)).toEqual(['불가', '권법']);
      expect(byTitle.get('무당')?.map((tag) => tag.tag)).toEqual(['도가']);
    });
  });
  ...
  describe('deleteWorldEntry', () => {
    ...
    it('should delete dependent tags and links', async () => {
      const source = await createWorldEntry(db, {
        projectId,
        category: '장소',
        title: '삭제 대상',
      });
      const target = await createWorldEntry(db, {
        projectId,
        category: '장소',
        title: '연결 대상',
      });
      db.insert(schema.worldEntryTags)
        .values({ entryId: source.id, tag: '중요' })
        .run();
      db.insert(schema.worldEntryLinks)
        .values({ sourceId: source.id, targetId: target.id })
        .run();

      await deleteWorldEntry(db, source.id);

      expect(
        db
          .select()
          .from(schema.worldEntryTags)
          .where(eq(schema.worldEntryTags.entryId, source.id))
          .all()
      ).toHaveLength(0);
      expect(
        db
          .select()
          .from(schema.worldEntryLinks)
          .where(eq(schema.worldEntryLinks.sourceId, source.id))
          .all()
      ).toHaveLength(0);
    });
  });
