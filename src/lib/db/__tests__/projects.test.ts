...
describe('Project Queries', () => {
  ...
  describe('listProjectSummaries', () => {
    it('should return only the fields rendered by the home library', async () => {
      await createProject(db, {
        title: '가벼운 홈 카드',
        genre: '판타지',
        synopsis: '홈 화면에 표시할 줄거리',
        settingsJson: JSON.stringify({ large: 'x'.repeat(10_000) }),
        writingStyleDescription: '홈에서는 불필요한 문체 정보',
        writingStyleSample: '홈에서는 불필요한 장문 샘플',
      });

      const result = await listProjectSummaries(db);

      expect(result).toHaveLength(1);
      expect(Object.keys(result[0]).sort()).toEqual(
        ['createdAt', 'genre', 'id', 'synopsis', 'title'].sort()
      );
    ...
    it('should retain newest-first ordering', async () => {
      const older = await createProject(db, { title: '이전 작품' });
      const newer = await createProject(db, { title: '최근 작품' });
      db.update(schema.projects)
        .set({ createdAt: new Date(Date.now() + 1000) })
        .where(eq(schema.projects.id, newer.id))
        .run();

      const result = await listProjectSummaries(db);

      expect(result.map((project) => project.id)).toEqual([newer.id, older.id]);
    });
  });
  ...
  describe('updateProject', () => {
    ...
    it('should clear optional project fields with null', async () => {
      ...
      expect(updated!.synopsis).toBeNull();
    });
