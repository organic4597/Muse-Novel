...
describe('Chapter Queries', () => {
  ...
  describe('deleteChapter', () => {
    ...
    it('should delete dependent character emotions with the chapter', async () => {
      const chapter = await createChapter(db, { projectId, title: '감정 기록 장' });
      const [character] = db
        .insert(schema.characters)
        .values({ projectId, name: '테스트 인물' })
        .returning()
        .all();
      db.insert(schema.characterEmotions)
        .values({
      ...
      const emotions = db
        .select()
        .from(schema.characterEmotions)
        .where(eq(schema.characterEmotions.chapterId, chapter.id))
        .all();
