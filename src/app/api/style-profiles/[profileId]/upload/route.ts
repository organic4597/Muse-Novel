...
export async function POST(
  ...
  if (!(file instanceof File)) {
    return NextResponse.json({ error: '텍스트 파일이 필요합니다.' }, { status: 400 });
  }
  ...
  const uploadDir = path.join(process.cwd(), 'public', 'uploads', 'styles', 'global', profileId);
  ...
  const filePath = `/uploads/styles/global/${profileId}/${filename}`;
