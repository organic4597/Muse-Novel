...
import { storylineNoteSummaryRequestSchema } from '@/lib/storyline-chat';
import { resolveProjectProvider } from '@/lib/ai/resolve-project-provider';
import { summarizeStorylineForNote } from '@/lib/ai/storyline-assistant';
...
export async function POST(request: Request, { params }: Context) {
  try {
    ...
    const body = storylineNoteSummaryRequestSchema.parse(await request.json());
    ...
    if (!answer || answer.role !== 'assistant') throw new AuthorNotebookError('정리할 AI 답변을 찾을 수 없습니다.', 404);
    const question = [...conversation.messages.slice(0, answerIndex)].reverse().find(message => message.role === 'user');
    if (!question) throw new AuthorNotebookError('답변에 연결된 작가의 질문을 찾을 수 없습니다.', 409);
    ...
    return longTaskResponse(request, async (signal, progress) => ({
      text: await summarizeStorylineForNote({
        projectId: id,
