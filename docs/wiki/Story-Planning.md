...
- 내용 없음: 플레이스홀더 표시
- 내용 있음: 구조화 draft 표시
- 적용 버튼: 제목과 핵심 기획 정보가 부족하면 비활성화

## 최근 UX 반영

- 인라인 AI 설정 제거, `/settings/ai` 사용으로 통합
- 반복 질문을 줄이기 위한 fallback / phase follow-up 보강
- skip/pass 입력을 다음 단계 진행 신호로 해석
- 거절된 pending 후보를 전체 재생성하지 않고 개별 교체
- 세계관 확정 항목 삭제 지원
- assistant options가 비어도 phase 기반 기본 추천 옵션 제공
